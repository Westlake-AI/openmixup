...
class AutoMixup_V2(BaseModel):
    ...
    *** Requiring Hook: `momentum_update` is adjusted by `CosineScheduleHook` after
    ...
    Args:
        ...
        mask_input_size (int or list): Image input size of MixBlock (training and testing). Build
            a list for various input_size. Default: None.
        ...
        lam_margin (int): Margin of lambda to stop using AutoMix to train backbone
            ...
            Default: -1 (or 0).
        HEM_lam_thr (float): Fix mixup lam<=0.5 and perform one-hot classification as ``Hard
            Example Mining (HEM)``. For example, when OHM_lam_thr=0.1, given (y_a, y_b, lam):
                y_mix = 0.1*y_a + 0.9*y_b as fixed target;
                x_mix = MixBlock(z_a, z_b, lam) with 0.1*2<lam<=1, lam = 0.1*2 + (1-0.1*2) * lam.
            Default: 0 (not using this trick).
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 **kwargs):
        super(AutoMixup_V2, self).__init__(init_cfg, **kwargs)
        # basic params
        ...
        self.mask_input_size = mask_input_size \
            if isinstance(mask_input_size, (int, list)) else None
        ...
        self.HEM_lam_thr = float(HEM_lam_thr) \
            if isinstance(HEM_lam_thr, (int, float)) else None
        ...
        assert 0 <= self.HEM_lam_thr < 0.5
        if self.mask_input_size is not None:
            if isinstance(self.mask_input_size, int):
                self.mask_input_size = [self.mask_input_size]
    ...
    def forward_train(self, img, gt_label, **kwargs):
        ...
        if self.HEM_lam_thr > 0:
            ...
            lam[1] = _thr + (1-_thr) * lam[1]  # (0, 1]
            # #########################
            # print("HEM_lam_thr={}, new_lam={}".format(self.HEM_lam_thr, lam[1]))
            # #########################

        # automatic pixel-wise mixup with feat
        ...
        results = self.pixel_mixup(img, gt_label, lam, indices, feature, affine_scale=affine_scale)
    ...
    def forward_q(self, x, mixed_x, y, index, lam):
        ...
        if self.weight_mix_q > 0:
            ...
            # mixup loss
            if self.HEM_lam_thr > 0:  # HEM onehot loss, y for 1-lam
                y_mix_q = y[index]
            else:
                # original mixup loss
                y_mix_q = (y, y[index], lam)
            # ###################################
            # if not isinstance(y_mix_q, tuple):
            #     print("y:", y_mix_q.size())
            # ###################################
    ...
    def pixel_mixup(self, x, y, lam, index, feature, affine_scale=1):
        """ pixel-wise input space mixup, v08.24
            v01.09, update AutoMix.V2
        ...
        Args:
            ...
            affine_scale (float): Affine transform based on the input_size and
                mask input_size. Default: 1.
        ...
        if x.shape[3] > 64:  # normal version of resnet
            scale_factor = 2**(2 + self.mask_layer) * max(1, affine_scale)
        else:  # CIFAR version
            scale_factor = 2**self.mask_layer * max(1, affine_scale)
        # ########################################
        # print("affine_scale={}, scale_factor={}".format(affine_scale, scale_factor))
        # ########################################
