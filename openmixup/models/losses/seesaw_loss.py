...
def seesaw_ce_loss(cls_score,
    ...
    cls_score = cls_score + (seesaw_weights.log() * (1 - labels))
...
class SeesawLoss(nn.Module):
    ...
    def forward(self,
        ...
        for u_l in unique_labels:
            inds_ = target == u_l.item()
            self.cum_samples[u_l] += inds_.sum()

        # convert target to onehot formats
        ...
        if weight is not None:
            ...
            weight = target.new_ones(target.size(), dtype=torch.float)
