...
model = dict(
    ...
    head_mix=dict(  # backbone mixup
        ...
        loss=dict(type='CrossEntropyLoss', loss_weight=1.0,
            use_soft=True, use_sigmoid=False, use_mix_decouple=True,  # try soft CE decouple
        ),
        with_avg_pool=True, multi_label=True, two_hot=False, two_hot_scale=1,  # no two-hot
        neg_weight=1,  # no neg
        lam_scale_mode='pow', lam_thr=1, lam_idx=0.5,  # lam rescale, default 'pow'
        eta_weight=dict(eta=0.5, mode="less", thr=0.5),  # eta for decouple mixup
        in_channels=512, num_classes=200),
    head_mix_k=dict(  # mixblock
        type='ClsMixupHead',  # mixup, soft CE
        loss=dict(type='CrossEntropyLoss', loss_weight=1.0,
            use_soft=True, use_sigmoid=False, use_mix_decouple=False,  # soft CE
        ),
        with_avg_pool=True, multi_label=True, two_hot=False, two_hot_scale=1,  # no two-hot
        neg_weight=0.5,  # try neg
        in_channels=512, num_classes=200),
