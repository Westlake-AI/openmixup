...
model = dict(
    ...
    pretrained=None,
    alpha=[1.0, 1.0],
    mix_mode=["transmix", "cutmix"],  # using TokenMix and CutMix
    mix_prob=[0.5, 0.5],
    debug=True,
    mix_args=dict(
        ...
        tokenmix=dict(mask_type='block', minimum_tokens=7),
        smmix=dict(side=7, min_side_ratio=0.25, max_side_ratio=0.75),
    ),
    backbone=dict(
